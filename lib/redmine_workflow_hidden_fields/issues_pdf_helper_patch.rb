module RedmineWorkflowHiddenFields
    module  IssuesPdfHelperPatch
		def self.included(base)
			...
			base.class_eval do
				unloadable
				alias_method_chain :issue_to_pdf, :hidden
				alias_method_chain :fetch_row_values, :hidden
			end
		end	
		
		module InstanceMethods		
			# Returns a PDF string of a single issue
			def issue_to_pdf_with_hidden(issue, assoc={})
...
#Modifications start around here				
				stdcols = []
				stdcols << [l(:field_status), issue.status] unless issue.hidden_attribute?('status')
				stdcols << [l(:field_priority), issue.priority] unless issue.hidden_attribute?('priority_id')
				stdcols << [l(:field_assigned_to), issue.assigned_to] unless issue.disabled_core_fields.include?('assigned_to_id') or issue.hidden_attribute?('assigned_to_id')
				stdcols << [l(:field_category), issue.category] unless issue.disabled_core_fields.include?('category_id') or issue.hidden_attribute?('category_id')
				stdcols << [l(:field_fixed_version), issue.fixed_version] unless issue.disabled_core_fields.include?('fixed_version_id') or issue.hidden_attribute?('fixed_version_id')

				
				stdcols << [l(:field_start_date), format_date(issue.start_date)] unless issue.disabled_core_fields.include?('start_date') or issue.hidden_attribute?('start_date')
				stdcols << [l(:field_due_date), format_date(issue.due_date)] unless issue.disabled_core_fields.include?('due_date') or issue.hidden_attribute?('due_date')
				stdcols << [l(:field_done_ratio), "#{issue.done_ratio}%"] unless issue.disabled_core_fields.include?('done_ratio') or issue.hidden_attribute?('done_ratio')
				stdcols << [l(:field_estimated_hours), l_hours(issue.estimated_hours)] unless issue.disabled_core_fields.include?('estimated_hours') or issue.hidden_attribute?('estimated_hours')
				stdcols << [l(:label_spent_time), l_hours(issue.total_spent_hours)] if User.current.allowed_to?(:view_time_entries, issue.project)
				
				left = []
				right = []
				stdcols.each_with_index do |value, i|
					if i % 2 == 0
						left << value
					else
						right << value
...
#Modifications end here
			...
			# fetch row values
			def fetch_row_values_with_hidden(issue, query, level)
